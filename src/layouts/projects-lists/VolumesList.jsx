...
const VolumesList = ({
    ...
    return (
        <>
            <div
                ref={wrapperRef}
                className={`work-list volume-list-wrapper cursor-pointer ${tailwindSlideTrans ? 'delay-[0.2s] duration-500 ease-in-out md:transition-[transform]' : ''} ${className} ${isOnIndexPage ? 'pointer-events-auto cursor-pointer' : 'w-full'} ${isSlugPage ? 'pointer-events-none' : ''}${isOnVolumePage ? '' : 'max-md:pointer-events-none'}`}
                style={{
